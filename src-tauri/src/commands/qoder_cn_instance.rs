...
fn resolve_running_pid(last_pid: Option<u32>, user_data_dir: Option<&str>) -> Option<u32> {
    if let Some(pid) = modules::process::resolve_qoder_cn_pid(last_pid, user_data_dir) {
        return Some(pid);
    }
...
fn inject_bound_account_for_instance_start(
    ...
    modules::instance_fingerprint::sync_qoder_family_active_profile_on_start(
        modules::qoder_account::QoderPlatformKind::Cn,
        instance_id,
    ...
    let account = modules::qoder_account::load_account_cn(bind_id)
        .ok_or_else(|| format!("绑定账号不存在: {}", bind_id))?;
    modules::logger::log_info(&format!(
        "[Qoder CN Instance] 实例启动检测到绑定账号，准备注入: bind_account_id={}, email={}, user_data_dir={}",
        bind_id, account.email, user_data_dir
    ));

    modules::qoder_account::inject_to_qoder_cn_for_user_data_dir(user_data_dir, bind_id)?;
    Ok(())
}

#[tauri::command]
pub async fn qoder_cn_get_instance_defaults() -> Result<modules::instance::InstanceDefaults, String> {
    modules::qoder_instance::get_instance_defaults_cn()
}

#[tauri::command]
pub async fn qoder_cn_list_instances() -> Result<Vec<InstanceProfileView>, String> {
    let store = modules::qoder_instance::load_instance_store_cn()?;
    let default_dir = modules::qoder_instance::get_default_qoder_user_data_dir_cn()?;
...
#[tauri::command]
pub async fn qoder_cn_create_instance(
    name: String,
    ...
    let instance =
        modules::qoder_instance::create_instance_cn(modules::qoder_instance::CreateInstanceParams {
            working_dir: None,
            name,
            user_data_dir,
...
#[tauri::command]
pub async fn qoder_cn_update_instance(
    instance_id: String,
    name: Option<String>,
    extra_args: Option<String>,
    ...
    if instance_id == DEFAULT_INSTANCE_ID {
        let default_dir = modules::qoder_instance::get_default_qoder_user_data_dir_cn()?;
        let default_dir_str = default_dir.to_string_lossy().to_string();
        let updated = modules::qoder_instance::update_default_settings_cn(
            bind_account_id,
            extra_args,
    ...
    let instance =
        modules::qoder_instance::update_instance_cn(modules::qoder_instance::UpdateInstanceParams {
            working_dir: None,
            instance_id,
            name,
            extra_args,
...
#[tauri::command]
pub async fn qoder_cn_delete_instance(instance_id: String) -> Result<(), String> {
    if instance_id == DEFAULT_INSTANCE_ID {
        return Err("默认实例不可删除".to_string());
    }
    modules::qoder_instance::delete_instance_cn(&instance_id)
}

#[tauri::command]
pub async fn qoder_cn_start_instance(instance_id: String) -> Result<InstanceProfileView, String> {
    modules::process::ensure_qoder_cn_launch_path_configured()?;

    if instance_id == DEFAULT_INSTANCE_ID {
        let default_dir = modules::qoder_instance::get_default_qoder_user_data_dir_cn()?;
        let default_dir_str = default_dir.to_string_lossy().to_string();
        let default_settings = modules::qoder_instance::load_default_settings_cn()?;

        if let Some(pid) = resolve_running_pid(default_settings.last_pid, None) {
            modules::process::close_pid(pid, 20)?;
            let _ = modules::qoder_instance::update_default_pid_cn(None)?;
        }
        modules::process::close_qoder_cn_instances(&[default_dir_str.clone()], 20)?;
        let _ = modules::qoder_instance::update_default_pid_cn(None)?;
        ...
        let pid =
            modules::process::start_qoder_cn_default_with_args_with_new_window(&extra_args, true)?;
        let _ = modules::qoder_instance::update_default_pid_cn(Some(pid))?;
    ...
    if let Some(pid) = resolve_running_pid(instance.last_pid, Some(&instance.user_data_dir)) {
        modules::process::close_pid(pid, 20)?;
        let _ = modules::qoder_instance::update_instance_pid_cn(&instance.id, None)?;
    }
    modules::process::close_qoder_cn_instances(&[instance.user_data_dir.clone()], 20)?;
    let _ = modules::qoder_instance::update_instance_pid_cn(&instance.id, None)?;
    let _ = modules::qoder_instance::update_default_pid_cn(None)?;
    ...
    let pid = modules::process::start_qoder_cn_with_args_with_new_window(
        &instance.user_data_dir,
        &extra_args,
        true,
    )?;

    let updated = modules::qoder_instance::update_instance_after_start_cn(&instance.id, pid)?;
...
#[tauri::command]
pub async fn qoder_cn_stop_instance(instance_id: String) -> Result<InstanceProfileView, String> {
    if instance_id == DEFAULT_INSTANCE_ID {
        let default_dir = modules::qoder_instance::get_default_qoder_user_data_dir_cn()?;
        let default_dir_str = default_dir.to_string_lossy().to_string();
        let default_settings = modules::qoder_instance::load_default_settings_cn()?;
    ...
    modules::process::close_qoder_cn_instances(&[instance.user_data_dir.clone()], 20)?;
    let updated = modules::qoder_instance::update_instance_pid_cn(&instance.id, None)?;
    let initialized = is_profile_initialized(&updated.user_data_dir);
    Ok(InstanceProfileView::from_profile(
        updated,
        false,
...
#[tauri::command]
pub async fn qoder_cn_open_instance_window(instance_id: String) -> Result<(), String> {
    if instance_id == DEFAULT_INSTANCE_ID {
        let default_settings = modules::qoder_instance::load_default_settings_cn()?;
        let pid = resolve_running_pid(default_settings.last_pid, None).ok_or("默认实例未运行")?;
        modules::process::focus_process_pid(pid)
            .map_err(|err| format!("定位 Qoder 国内版默认实例窗口失败: {}", err))?;
        return Ok(());
    }
    ...
    modules::process::focus_process_pid(pid).map_err(|err| {
        format!(
            "定位 Qoder 国内版实例窗口失败: instance_id={}, err={}",
            instance.id, err
        )
    })?;
    Ok(())
...
#[tauri::command]
pub async fn qoder_cn_close_all_instances() -> Result<(), String> {
    let store = modules::qoder_instance::load_instance_store_cn()?;
    let default_dir = modules::qoder_instance::get_default_qoder_user_data_dir_cn()?;
    ...
    modules::process::close_qoder_cn_instances(&target_dirs, 20)?;
    let _ = modules::qoder_instance::clear_all_pids_cn();
    Ok(())
}
