...
pub async fn import_qoder_from_local(app: AppHandle) -> Result<Vec<QoderAccount>, String> {
    match qoder_account::import_from_local()? {
        Some(account) => {
            let final_account = match qoder_oauth::refresh_account_from_openapi(&account.id).await {
...
pub async fn inject_qoder_account(app: AppHandle, account_id: String) -> Result<String, String> {
    ...
    let default_dir = crate::modules::qoder_instance::get_default_qoder_user_data_dir()
        .map(|d| d.to_string_lossy().to_string())
        .unwrap_or_default();
    let _ = crate::modules::process::close_qoder_instances(&[default_dir], 20);
...
#[tauri::command]
pub async fn claim_qoder_checkin(account_id: Option<String>) -> Result<serde_json::Value, String> {
    let target_id = match account_id.filter(|s| !s.trim().is_empty()) {
        Some(id) => id,
        None => qoder_account::list_accounts_for_platform(qoder_account::QoderPlatformKind::Global)
            .into_iter()
            .next()
            .map(|a| a.id)
            .ok_or_else(|| "暂无 Qoder 账号可签到".to_string())?,
    };
    crate::modules::qwenwork_account::claim_qoder_campaign_checkin(
        qoder_account::QoderPlatformKind::Global,
        &target_id,
    )
    .await
}
...
#[tauri::command]
pub async fn get_qoder_checkin_status(account_id: String) -> Result<serde_json::Value, String> {
    crate::modules::qwenwork_account::get_qoder_campaign_checkin_status(
        qoder_account::QoderPlatformKind::Global,
        &account_id,
    )
    .await
