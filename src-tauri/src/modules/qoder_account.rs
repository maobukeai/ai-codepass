...
pub fn record_platform_checkin_reward(
    ...
    let mut dates: Vec<String> = credit_obj
        .get("checkin_dates")
        .and_then(|v| v.as_array())
        .map(|arr| arr.iter().filter_map(|s| s.as_str().map(|str| str.to_string())).collect())
        .unwrap_or_default();
    if !dates.contains(&today.to_string()) {
        dates.insert(0, today.to_string());
    }
    ...
    credit_obj.insert("checkin_dates".to_string(), serde_json::json!(dates));
    credit_obj.insert("streak_days".to_string(), serde_json::json!(streak));
    account.auth_credit_usage_raw = Some(Value::Object(credit_obj));

    upsert_account_record_for_platform(kind, account)
}

pub fn calculate_streak_days(dates: &[String], today: &str) -> i64 {
    if dates.is_empty() {
        return 0;
    }
    ...
    let today_date = match chrono::NaiveDate::parse_from_str(today, "%Y-%m-%d") {
        Ok(d) => d,
        Err(_) => return parsed_dates.len().min(1) as i64,
    };

    let mut current_target = if parsed_dates.contains(&today_date) {
        today_date
    } else {
    ...
    for d in &parsed_dates {
        if *d == current_target {
            streak += 1;
            current_target = current_target - chrono::Duration::days(1);
        } else if *d > current_target {
            continue;
        } else {
            break;
        }
...
pub async fn sync_qoder_usage_from_remote(
    ...
    if let Some(usage_obj) = body.get("qoderUsage") {
        let mut merged_usage = usage_obj.clone();
        if let Some(existing_map) = account.auth_credit_usage_raw.as_ref().and_then(|v| v.as_object()) {
            if let Some(new_map) = merged_usage.as_object_mut() {
                for key in &[
                    "last_checkin_date",
                    "checked_in_today",
                    "checkin_reward",
                    ...
                    if !new_map.contains_key(*key) {
                        if let Some(val) = existing_map.get(*key) {
                            new_map.insert(key.to_string(), val.clone());
                        }
                    }
                }
            }
        }
        account.auth_credit_usage_raw = Some(merged_usage);
