...
pub async fn run_workbuddy_auto_checkin_cycle_if_needed(
    ...
    let mut accounts = workbuddy_account::list_accounts();
    let mut ai_accounts = crate::modules::workbuddy_ai_account::list_accounts();
    for acc in &mut ai_accounts {
        if acc.domain.is_none() {
            acc.domain = Some("www.workbuddy.ai".to_string());
        }
    }
    accounts.extend(ai_accounts);
        ...
        .await
        {
            ...
            .await
            {
                Ok(res) if res.success => {
                    ...
                    if account.domain.as_deref() == Some("www.workbuddy.ai") || account.id.starts_with("wb_ai_") {
                        let _ = crate::modules::workbuddy_ai_account::update_checkin_info(
                            &account.id,
    ...
    for d in &details {
        ...
        let msg = d.message.as_deref().unwrap_or("处理完毕");
        detail_lines.push_str(&format!("> • **{}**：{} {}\n", d.email, icon, msg));
    }
    if detail_lines.is_empty() {
        detail_lines.push_str("> • 无待处理账号\n");
    }

    let summary_text = format!(
        "### 📊 签到统计\n- **执行日期**：{}\n- **处理账号**：{} 个账号\n- **执行结果**：成功 {} | 今日已签 {} | 失败 {}\n- **执行耗时**：{} ms\n\n### 📋 账号明细\n{}\n---\n*AI CodePass 每日自动为您守护算力额度*",
        today_str, target_account_count, success_count, already_checked_count, failed_count, duration_ms, detail_lines
    );
    tokio::spawn(async move {
        crate::modules::webhook_notify::send_auto_checkin_notification(
            "🎉 [AI CodePass] CodeBuddy 自动打卡汇报",
            &summary_text,
        ).await;
    });
