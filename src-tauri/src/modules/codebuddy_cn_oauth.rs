...
fn resolve_checkin_endpoint(domain: Option<&str>) -> &str {
    match domain {
        Some(d) if d.contains("workbuddy.ai") => "https://www.workbuddy.ai",
        _ => CODEBUDDY_API_ENDPOINT,
    }
}
...
async fn fetch_checkin_status_from(
    ...
    let client = build_client()?;
    let endpoint = resolve_checkin_endpoint(domain);
...
pub async fn perform_checkin(
    ...
    let client = build_client()?;
    let endpoint = resolve_checkin_endpoint(domain);
    let url = format!("{}/v2/billing/meter/daily-checkin", endpoint);
