...
fn normalize_platform(platform: &str) -> Result<&'static str, String> {
    match platform.trim() {
        ...
        "qwenwork" | "qwen_work" | "qwen-work" => Ok("qwenwork"),
        "zcode" => Ok("zcode"),
        "trae" => Ok("trae"),
        ...
        "workbuddy_ai" | "workbuddy-ai" => Ok("workbuddy_ai"),
...
mod tests {
    ...
    fn provider_current_state_normalizes_platform_aliases() {
        ...
        set_current_account_id("qoder-cn", Some("qoder-cn-account")).expect("set qoder cn alias");
        assert_eq!(
            get_current_account_id("qoder_cn").expect("get qoder cn canonical"),
            Some("qoder-cn-account".to_string())
        );
