...
pub fn write_account_to_default_client(account: &WorkbuddyAccount) -> Result<(), String> {
    ...
    if auth_file.exists() {
        ...
        // 备份现有登录信息，以防万一
        let backup_path = auth_file.with_extension("info.bak");
        let _ = fs::write(&backup_path, &existing);

        let has_encrypted = contains_encrypted_wrapper(&existing_json);
        ...
        let written = crate::modules::atomic_write::write_string_atomic_if_hash_matches(
            ...
            expected_hash,
            || {
                let session = if has_encrypted {
                    logger::log_info(
                        "[WorkbuddyAccount] 现有登录文件包含官方加密字段，执行纯净凭据安全覆盖切换",
                    );
                    build_default_client_auth_session(account)
                } else {
