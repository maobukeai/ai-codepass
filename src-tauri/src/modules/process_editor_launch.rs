...
pub fn start_qoder_cn_with_args_with_new_window(
    user_data_dir: &str,
    extra_args: &[String],
    use_new_window: bool,
) -> Result<u32, String> {
    #[cfg(target_os = "macos")]
    {
        ...
        let launch_path = resolve_qoder_cn_launch_path()?;
        let app_root = resolve_macos_app_root_from_launch_path(&launch_path)
            .ok_or_else(|| app_path_missing_error("qoder_cn"))?;
        ...
        crate::modules::logger::log_info("Qoder CN 启动命令已发送（open -n -a）");
        let probe_started = Instant::now();
        let timeout = Duration::from_secs(6);
        while probe_started.elapsed() < timeout {
            if let Some(resolved_pid) = resolve_qoder_cn_pid(None, Some(target)) {
                return Ok(resolved_pid);
            }
            thread::sleep(Duration::from_millis(200));
        }
        crate::modules::logger::log_warn(&format!(
            "[Qoder CN Start] 启动后 6s 内未匹配到实例 PID，回退 open pid={}",
            open_pid
        ));
        return Ok(open_pid);
    }
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        let child =
            spawn_command_with_trace(&mut cmd).map_err(|e| format!("启动 Qoder CN 失败: {}", e))?;
        crate::modules::logger::log_info("Qoder CN 启动命令已发送");
        return Ok(child.id());
    }
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let child = spawn_detached_unix(&mut cmd).map_err(|e| format!("启动 Qoder CN 失败: {}", e))?;
        crate::modules::logger::log_info("Qoder CN 启动命令已发送");
        return Ok(child.id());
    }
    ...
    {
        let _ = (user_data_dir, extra_args, use_new_window);
        Err("Qoder CN 应用多开仅支持 macOS、Windows 和 Linux".to_string())
    }
}

pub fn start_qoder_cn_default_with_args_with_new_window(
    extra_args: &[String],
    use_new_window: bool,
) -> Result<u32, String> {
    #[cfg(target_os = "macos")]
    {
        ...
        let open_pid = spawn_open_app_with_options(&app_root, &args, true)
            .map_err(|e| format!("启动 Qoder CN 失败: {}", e))?;
        crate::modules::logger::log_info("Qoder CN 默认实例启动命令已发送（open -n -a）");
        let probe_started = Instant::now();
        let timeout = Duration::from_secs(6);
        while probe_started.elapsed() < timeout {
            if let Some(resolved_pid) = resolve_qoder_cn_pid(None, None) {
                return Ok(resolved_pid);
            }
            thread::sleep(Duration::from_millis(200));
        }
        crate::modules::logger::log_warn(&format!(
            "[Qoder CN Start] 启动后 6s 内未匹配到默认实例 PID，回退 open pid={}",
            open_pid
        ));
        return Ok(open_pid);
    }
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        let child =
            spawn_command_with_trace(&mut cmd).map_err(|e| format!("启动 Qoder CN 失败: {}", e))?;
        crate::modules::logger::log_info("Qoder CN 默认实例启动命令已发送");
        return Ok(child.id());
    }
