...
fn detect_qoder_cn_exec_path() -> Option<std::path::PathBuf> {
    #[cfg(target_os = "macos")]
    {
        let candidates = [
            ...
            "/Applications/QoderCN.app",
            "/Applications/Qoder-CN.app/Contents/MacOS/Qoder",
...
fn linux_codex_discovery_paths(
    ...
    if let Some(path_env) = path_env {
        ...
        let dirs: Vec<PathBuf> = if path_str.contains(':') && !path_str.contains(';') {
...
pub fn ensure_qoder_cn_launch_path_configured() -> Result<(), String> {
    resolve_qoder_cn_launch_path().map(|_| ())
}
...
fn resolve_qoder_cn_launch_path() -> Result<std::path::PathBuf, String> {
    if let Some(custom) = normalize_custom_path(Some(&config::get_user_config().qoder_cn_app_path)) {
        if let Some(exec) = resolve_qoder_macos_exec_path(&custom) {
            return Ok(exec);
        }
        return Err(app_path_missing_error("qoder_cn"));
    }

    if let Some(detected) = detect_qoder_cn_exec_path() {
        let detected_str = detected.to_string_lossy();
        if let Some(exec) = resolve_qoder_macos_exec_path(&detected_str) {
            return Ok(exec);
        }
    ...
    Err(app_path_missing_error("qoder_cn"))
}
...
fn detect_and_save_app_path_raw(app: &str, force: bool) -> Option<String> {
    ...
    match app {
        ...
        "qoder_cn" => {
            if !force && !current.qoder_cn_app_path.trim().is_empty() {
                return Some(current.qoder_cn_app_path);
            }
            if let Some(detected) = detect_qoder_cn_exec_path() {
                update_app_path_in_config("qoder_cn", &detected, &current.qoder_cn_app_path);
                return Some(config::get_user_config().qoder_cn_app_path);
            }
        }
        "zcode" => {
