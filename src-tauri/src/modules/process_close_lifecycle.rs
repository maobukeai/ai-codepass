...
pub fn close_qoder_cn_instances(user_data_dirs: &[String], timeout_secs: u64) -> Result<(), String> {
    let default_dir = get_default_qoder_cn_user_data_dir_for_os()
        .map(|value| normalize_path_for_compare(&value))
        .filter(|value| !value.is_empty());
    close_user_data_dir_scoped_instances(
        "Qoder CN Close",
        "Qoder CN",
        "Unable to close managed Qoder CN instances; please close them manually and retry",
        user_data_dirs,
        timeout_secs,
        default_dir,
        collect_qoder_cn_process_entries,
    )
}
