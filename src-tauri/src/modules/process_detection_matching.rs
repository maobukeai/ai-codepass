...
fn resolve_expected_qoder_cn_launch_path_for_match() -> Option<String> {
    let launch_path = match resolve_qoder_cn_launch_path() {
        Ok(path) => path,
        Err(err) => {
            crate::modules::logger::log_warn(&format!(
                "[Qoder CN Resolve] launch path missing or invalid, skip PID match: {}",
                err
            ));
            return None;
        }
    ...
    let normalized = normalize_path_for_compare(launch_path.to_string_lossy().as_ref());
    if normalized.is_empty() {
        crate::modules::logger::log_warn("[Qoder CN Resolve] launch path is empty, skip PID match");
        return None;
    }
    Some(normalized)
}
...
fn resolve_qoder_cn_target_and_fallback(user_data_dir: Option<&str>) -> Option<(String, bool)> {
    build_user_data_dir_match_target(
        user_data_dir,
        get_default_qoder_cn_user_data_dir_for_os(),
        !strict_process_detect_enabled(),
    )
}
...
#[cfg(target_os = "windows")]
fn get_managed_codex_windows_app_user_data_dir(_codex_home: &str) -> Option<String> {
    None
}
...
pub fn resolve_qoder_cn_pid_from_entries(
    last_pid: Option<u32>,
    user_data_dir: Option<&str>,
    entries: &[(u32, Option<String>)],
) -> Option<u32> {
    let (target, allow_none_for_target) = resolve_qoder_cn_target_and_fallback(user_data_dir)?;
    resolve_pid_from_entries_by_user_data_dir(last_pid, &target, allow_none_for_target, entries)
}

pub fn resolve_qoder_cn_pid(last_pid: Option<u32>, user_data_dir: Option<&str>) -> Option<u32> {
    let entries = collect_qoder_cn_process_entries();
    resolve_qoder_cn_pid_from_entries(last_pid, user_data_dir, &entries)
}
...
pub fn collect_qoder_cn_process_entries() -> Vec<(u32, Option<String>)> {
    let expected_launch = resolve_expected_qoder_cn_launch_path_for_match();
    if expected_launch.is_none() {
        return Vec::new();
    }
    ...
    #[cfg(target_os = "windows")]
    {
        let expected = expected_launch
            ...
            .expect("expected launch path must exist");
        let mut entries =
            collect_named_electron_process_entries_from_powershell(expected, "Qoder CN.exe", "Qoder CN");
        if entries.is_empty() {
            entries =
                collect_named_electron_process_entries_from_powershell(expected, "Qoder.exe", "Qoder");
        }
        ...
        crate::modules::logger::log_warn(
            "[Qoder CN Probe] PowerShell returned empty; fallback to sysinfo probe",
        );
    ...
    #[cfg(target_os = "macos")]
    {
        let entries = collect_qoder_process_entries_macos();
        if !entries.is_empty() {
            return filter_entries_by_expected_launch_path("Qoder CN", entries, expected_launch);
        }
        return Vec::new();
    }
...
fn get_default_qoder_cn_user_data_dir_for_os() -> Option<String> {
    crate::modules::qoder_instance::get_default_qoder_user_data_dir_for_platform(
