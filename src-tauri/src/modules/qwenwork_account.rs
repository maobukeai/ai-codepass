...
pub async fn claim_qoder_campaign_checkin(
    ...
    let updated = qoder_account::sync_qoder_usage_from_remote(kind, account_id, &token).await
        .unwrap_or_else(|_| {
            qoder_account::record_platform_checkin_reward(kind, account_id, &today, reward)
                .unwrap_or(account)
        });
        ...
        if let Ok(Some(v2)) = read_auth_v2_json(&data_dir) {
            if let Some(token) = v2.get("token").and_then(|v| v.as_str()) {
                ...
                if let Some(acc) = accounts.first() {
                    ...
                    assert_eq!(synced.display_name.as_deref(), Some("猫步可爱"));
