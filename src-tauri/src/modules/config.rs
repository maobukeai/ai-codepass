...
pub struct UserConfig {
    ...
    /// WorkBuddy AI 自动刷新间隔（分钟），-1 表示禁用
    #[serde(default = "default_workbuddy_ai_auto_refresh")]
    pub workbuddy_ai_auto_refresh_minutes: i32,
...
fn default_workbuddy_ai_auto_refresh() -> i32 {
    10
}
...
impl Default for UserConfig {
    fn default() -> Self {
        Self {
            ...
            workbuddy_ai_auto_refresh_minutes: default_workbuddy_ai_auto_refresh(),
...
const LOCAL_PROXY_BYPASS_HOSTS: &[&str] = &["localhost", "127.0.0.1", "::1"];
...
pub fn get_shared_dir() -> PathBuf {
    crate::modules::account::resolve_data_dir()
        .unwrap_or_else(|_| PathBuf::from(".ai_codepass"))
}
...
pub fn load_user_config() -> Result<UserConfig, String> {
    ...
    if let Some(obj) = value.as_object_mut() {
        ...
        if !obj.contains_key("qoder_cn_auto_refresh_minutes") {
            let inherited_refresh = obj
                .get("qoder_auto_refresh_minutes")
                .and_then(|v| v.as_i64())
                .map(|v| v as i32)
                .unwrap_or_else(default_qoder_cn_auto_refresh);
            obj.insert(
                "qoder_cn_auto_refresh_minutes".to_string(),
                json!(inherited_refresh),
            );
        }
        ...
        if !obj.contains_key("workbuddy_ai_auto_refresh_minutes") {
            let inherited_refresh = obj
                .get("workbuddy_auto_refresh_minutes")
                .or_else(|| obj.get("codebuddy_auto_refresh_minutes"))
                .and_then(|v| v.as_i64())
                .map(|v| v as i32)
                .unwrap_or_else(default_workbuddy_ai_auto_refresh);
            obj.insert(
                "workbuddy_ai_auto_refresh_minutes".to_string(),
                json!(inherited_refresh),
            );
        }
        ...
        if !obj.contains_key("qoder_cn_quota_alert_enabled") {
            obj.insert(
                "qoder_cn_quota_alert_enabled".to_string(),
                json!(legacy_enabled),
            );
        }
        if !obj.contains_key("qoder_cn_quota_alert_threshold") {
            obj.insert(
                "qoder_cn_quota_alert_threshold".to_string(),
                json!(legacy_threshold),
            );
        }
...
mod tests {
    ...
    fn load_user_config_turns_off_legacy_startup_minimized_once() {
        ...
        std::env::set_var("AI_CODEPASS_TEST_DATA_DIR", &data_dir);
...
"#,
        ...
        std::env::remove_var("AI_CODEPASS_TEST_DATA_DIR");
