...
fn resolve_checkin_device_id(account: &TraeAccount, passed_device_id: &str) -> String {
    ...
    if let Some(auth) = &account.trae_auth_raw {
        if let Some(dev_id) = auth.pointer("/deviceInfo/DeviceID").and_then(|v| v.as_str()) {
            if is_valid_numeric(dev_id) {
                return dev_id.to_string();
            }
        }
        if let Some(dev_id) = auth.get("deviceId").and_then(|v| v.as_str()) {
            if is_valid_numeric(dev_id) {
                return dev_id.to_string();
            }
        }
    ...
    let h = hasher.finish();
    format!("{:016}", 100_000_000_000_0000u64 + (h % 900_000_000_000_0000u64))
}
...
fn build_trae_checkin_headers(
    ...
    // Trae CN 签到 API 强制要求 Cloud-IDE-JWT 前缀
    headers.insert(
...
/// 执行单次签到状态查询
async fn request_checkin_status_raw(
    client: &reqwest::Client,
    token: &str,
    device_id: &str,
) -> Result<(reqwest::StatusCode, TraeCheckinStatusResponse, String), String> {
    let headers = build_trae_checkin_headers(token, device_id)?;
    let url = "https://api.trae.cn/trae/api/v2/ug/checkin_credits/status";
    let body_json = serde_json::json!({ "req_source": 1 });
    ...
    if !status.is_success() {
        return Err(format!("获取签到状态失败 (HTTP {}): {}", status, body));
    }

    let data: TraeCheckinStatusResponse =
        serde_json::from_str(&body).map_err(|e| format!("解析签到状态响应失败: {}, 原始响应: {}", e, body))?;

    Ok((status, data, body))
}

/// 获取 Trae 账号的今日签到状态（支持自动静默刷新 Token）
pub async fn get_trae_checkin_status(
    ...
    let platform = resolve_account_platform_kind(&account);
    if !platform.is_cn() {
        return Err("Trae 国际版采用月度免费额度自动重置模式，官方未开放每日签到活动。每日签到仅对 Trae 国内版开放。".to_string());
    }
    let resolved_did = resolve_checkin_device_id(&account, device_id);
    ...
    let mut current_token = account.access_token.clone();
    let mut did_refresh = false;

    let data = loop {
        match request_checkin_status_raw(&client, &current_token, &resolved_did).await {
            Ok((_, data, _)) => {
                if data.code == 1001 && !did_refresh {
                    logger::log_info(&format!(
                        "[Trae Checkin] 查询状态检测到 Token 过期 (code=1001)，自动触发静默刷新: account_id={}",
                        account_id
                    ));
                    did_refresh = true;
                    match refresh_account_async(account_id).await {
                        ...
                        Err(err) => {
                            return Err(format!("Trae 会话已过期，请在 Trae 重新登录后从本地导入: {}", err));
                        }
                    }
                }
                if data.code != 0 {
                    let msg = if !data.message.is_empty() {
                        data.message
                    } else {
                        format!("code={}", data.code)
                    };
                    return Err(format!("获取签到状态失败: {}", msg));
                }
                break data;
            }
            Err(err) => {
                if !did_refresh && (err.contains("1001") || err.contains("401")) {
                    logger::log_info(&format!(
                        "[Trae Checkin] 请求异常判定为鉴权失败，尝试刷新重试: account_id={}, err={}",
                        account_id, err
                    ));
                    did_refresh = true;
                    if let Ok(refreshed) = refresh_account_async(account_id).await {
                        current_token = refreshed.access_token;
                        continue;
                    }
                }
    ...
    let is_checked_in = data.checked_in || data.did_checked_in;
    let message = if is_checked_in {
...
/// 领取 Trae 账号的今日签到积分（支持自动静默刷新 Token 与 9074 限流智能重试）
pub async fn claim_trae_checkin(
    ...
    let mut current_token = account.access_token.clone();
    let mut did_refresh = false;
    let url = "https://api.trae.cn/trae/api/v2/ug/checkin_credits/claim";
    let body_json = serde_json::json!({ "req_source": 1 });

    let mut attempts = 0;
    loop {
        attempts += 1;
        let headers = build_trae_checkin_headers(&current_token, &resolved_did)?;
        ...
        if !status.is_success() {
            return Err(format!("签到领取失败 (HTTP {}): {}", status, body));
        }

        let claim_data: TraeCheckinClaimResponse = match serde_json::from_str(&body) {
            Ok(d) => d,
            Err(e) => {
                return Err(format!("解析签到领取响应失败: {}, 原始响应: {}", e, body));
            }
        };

        if claim_data.code == 1001 && !did_refresh {
            logger::log_info(&format!(
                "[Trae Checkin] 领取签到检测到 Token 过期 (code=1001)，自动触发静默刷新: account_id={}",
                account_id
            ));
            did_refresh = true;
            match refresh_account_async(account_id).await {
        ...
        // 9074: 当前参与用户太多，轻度退避重试 (最多重试 3 次)
        if claim_data.code == 9074 && attempts < 3 {
            logger::log_info(&format!(
                "[Trae Checkin] 命中 Trae 限流重试 (code=9074, attempt={}): 等待 2 秒后重试...",
                attempts
            ));
        ...
        if claim_data.code != 0 {
            let msg = if !claim_data.message.is_empty() {
                claim_data.message
            } else {
                format!("code={}", claim_data.code)
            };
            return Err(format!("签到领取失败: {}", msg));
        }
    ...
    // 领取成功后查询最新状态
    let status_result = get_trae_checkin_status(account_id, &resolved_did).await?;

    Ok(CheckinStatusResult {
        message: format!("签到成功！当前共拥有 {} 积分", status_result.total_credits),
        ..status_result
    })
}
