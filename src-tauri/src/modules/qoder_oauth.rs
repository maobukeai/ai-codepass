...
const DEFAULT_LOGIN_BASE_URL_CN: &str = "https://qoder.com.cn/device/selectAccounts";
const DEFAULT_OPENAPI_BASE_URL_CN: &str = "https://openapi.qoder.com.cn";
...
lazy_static::lazy_static! {
    ...
    static ref PENDING_OAUTH_STATE_CN: Arc<Mutex<Option<PendingOAuthState>>> = Arc::new(Mutex::new(None));
    static ref PENDING_OAUTH_STATE_QWENWORK: Arc<Mutex<Option<PendingOAuthState>>> = Arc::new(Mutex::new(None));
}

fn pending_oauth_state_for_platform(
    kind: QoderPlatformKind,
) -> &'static Arc<Mutex<Option<PendingOAuthState>>> {
    match kind {
        QoderPlatformKind::Global => &PENDING_OAUTH_STATE,
        QoderPlatformKind::Cn => &PENDING_OAUTH_STATE_CN,
        QoderPlatformKind::QwenWork => &PENDING_OAUTH_STATE_QWENWORK,
    }
}
...
fn resolve_qoder_cli_login_endpoint(kind: QoderPlatformKind) -> String {
    let base = match kind {
        QoderPlatformKind::Global => DEFAULT_LOGIN_BASE_URL,
        QoderPlatformKind::Cn | QoderPlatformKind::QwenWork => DEFAULT_LOGIN_BASE_URL_CN,
    };
    normalize_url_origin_and_path(base).unwrap_or_else(|| base.to_string())
}

fn resolve_qoder_openapi_base_url(kind: QoderPlatformKind) -> &'static str {
    match kind {
        QoderPlatformKind::Global => DEFAULT_OPENAPI_BASE_URL,
        QoderPlatformKind::Cn | QoderPlatformKind::QwenWork => DEFAULT_OPENAPI_BASE_URL_CN,
    }
}
...
fn detect_qoder_product_version(kind: QoderPlatformKind) -> Option<String> {
    let mut base_paths: Vec<PathBuf> = Vec::new();
    let configured_path = match kind {
        QoderPlatformKind::Global => config::get_user_config().qoder_app_path.trim().to_string(),
        QoderPlatformKind::Cn => config::get_user_config().qoder_cn_app_path.trim().to_string(),
        QoderPlatformKind::QwenWork => crate::modules::qwenwork_account::detect_qwenwork_exec_path()
            .map(|p| p.to_string_lossy().into_owned())
            .unwrap_or_else(|| config::get_user_config().qoder_cn_app_path.trim().to_string()),
    };
    ...
    #[cfg(target_os = "macos")]
    {
        match kind {
            ...
            QoderPlatformKind::Cn | QoderPlatformKind::QwenWork => {
                base_paths.push(PathBuf::from("/Applications/QoderCN.app"));
                base_paths.push(PathBuf::from(
                    "/Applications/QoderCN.app/Contents/MacOS/QoderCN",
                ));
                base_paths.push(PathBuf::from(
                    "/Applications/QoderCN.app/Contents/MacOS/Qoder",
                ));
                base_paths.push(PathBuf::from(
                    "/Applications/QoderCN.app/Contents/MacOS/Electron",
                ));
                base_paths.push(PathBuf::from("/Applications/Qoder-CN.app"));
                base_paths.push(PathBuf::from("/Applications/Qoder.app"));
            }
        }
    }
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        if let Ok(program_files) = std::env::var("ProgramFiles") {
            match kind {
                ...
                QoderPlatformKind::Cn | QoderPlatformKind::QwenWork => {
                    base_paths.push(PathBuf::from(&program_files).join("QoderCN").join("QoderCN.exe"));
                    base_paths.push(PathBuf::from(&program_files).join("QoderCN").join("Qoder.exe"));
                    base_paths.push(PathBuf::from(&program_files).join("Qoder-CN").join("Qoder.exe"));
                    base_paths.push(PathBuf::from(&program_files).join("Qoder").join("Qoder.exe"));
                }
            }
        }
...
fn build_qoder_headers(
    ...
    let cosy_version = machine_info
        ...
        .or_else(|| detect_qoder_product_version(kind));
    ...
    logger::log_info(&format!(
        "[{}] 构造请求头: has_cosy_version={}, has_machine_token={}, has_machine_type={}, machine_os={}",
        kind.log_prefix(),
...
async fn fetch_qoder_user_info(
    ...
    let headers = build_qoder_headers(kind, token, machine_info);
...
async fn fetch_qoder_user_status_bundle(
    ...
    let status_headers = build_qoder_headers(kind, token, machine_info);
...
async fn fetch_qoder_user_status_bundle_with_machine_info(
    ...
    fetch_qoder_user_status_bundle(kind, client, openapi_base_url, token, machine_info).await
}

async fn refresh_account_from_openapi_once_for_platform(
    kind: QoderPlatformKind,
    account_id: &str,
) -> Result<QoderAccount, String> {
    let target = qoder_account::load_account_for_platform(kind, account_id)
        .ok_or_else(|| format!("{} 账号不存在: {}", kind.display_name(), account_id))?;

    let access_token = extract_access_token_from_account(&target)
        .ok_or_else(|| format!("{} 账号缺少 access token，请重新登录后再刷新", kind.display_name()))?;

    let client = build_reqwest_client()?;
    let machine_info = match read_qoder_machine_info_cache_for_platform(kind) {
        Ok(value) => value,
        Err(err) => {
            logger::log_warn(&format!(
                "[{} Refresh] 读取官方 machine token 缓存失败，将继续尝试无机器标识链路: {}",
                kind.log_prefix(),
                err
            ));
            None
    ...
    let openapi_base_url = resolve_qoder_openapi_base_url(kind);
    ...
    .await
    {
        ...
        Err(err) => {
            logger::log_warn(&format!(
                "[{} Refresh] 获取 /api/v2/user/plan 失败，将沿用本地缓存: {}",
                kind.log_prefix(),
                err
            ));
    ...
    .await
    {
        ...
        Err(err) => {
            logger::log_warn(&format!(
                "[{} Refresh] 获取 /api/v2/quota/usage 失败，将沿用本地缓存: {}",
                kind.log_prefix(),
                err
            ));
    ...
    let refreshed = qoder_account::upsert_account_from_snapshot_for_platform(
        kind,
        user_info_raw,
        user_plan_raw,
        credit_usage_raw,
    ...
    let refreshed = if refreshed.id == target.id {
        qoder_account::update_quota_query_error_for_platform(kind, &refreshed.id, quota_query_error)?
            .unwrap_or(refreshed)
    } else {
        refreshed
    };
...
pub async fn refresh_account_from_openapi_for_platform(
    kind: QoderPlatformKind,
    account_id: &str,
) -> Result<QoderAccount, String> {
    let result = refresh_account_from_openapi_once_for_platform(kind, account_id).await;
    if let Err(err) = &result {
        let _ = qoder_account::update_quota_query_error_for_platform(kind, account_id, Some(err.clone()));
    }
    result
}

pub async fn refresh_all_accounts_from_openapi_for_platform(kind: QoderPlatformKind) -> Result<i32, String> {
    let accounts = qoder_account::list_accounts_for_platform(kind);
    ...
    for account in accounts {
        match refresh_account_from_openapi_for_platform(kind, &account.id).await {
            Ok(_) => {
                success_count += 1;
            }
            Err(err) => {
                logger::log_warn(&format!(
                    "[{} Refresh] 批量刷新失败: account_id={}, email={}, error={}",
                    kind.log_prefix(),
                    account.id,
                    account.email,
                    err
...
pub async fn refresh_account_from_openapi(account_id: &str) -> Result<QoderAccount, String> {
    refresh_account_from_openapi_for_platform(QoderPlatformKind::Global, account_id).await
}

pub async fn refresh_account_from_openapi_cn(account_id: &str) -> Result<QoderAccount, String> {
    refresh_account_from_openapi_for_platform(QoderPlatformKind::Cn, account_id).await
}

pub async fn refresh_all_accounts_from_openapi() -> Result<i32, String> {
    refresh_all_accounts_from_openapi_for_platform(QoderPlatformKind::Global).await
}

pub async fn refresh_all_accounts_from_openapi_cn() -> Result<i32, String> {
    refresh_all_accounts_from_openapi_for_platform(QoderPlatformKind::Cn).await
}

fn clear_pending_if_matches_for_platform(kind: QoderPlatformKind, login_id: &str) {
    if let Ok(mut guard) = pending_oauth_state_for_platform(kind).lock() {
...
fn default_cosy_info_path_for_platform(kind: QoderPlatformKind) -> Result<PathBuf, String> {
    let user_data = qoder_instance::get_default_qoder_user_data_dir_for_platform(kind)?;
    Ok(user_data.join("SharedClientCache"))
}

fn read_qoder_machine_info_cache_for_platform(kind: QoderPlatformKind) -> Result<Option<QoderMachineInfo>, String> {
    let cache_path = default_cosy_info_path_for_platform(kind)?
        .join("cache")
        .join("machine_token.json");
    if !cache_path.exists() {
        logger::log_warn(&format!(
            "[{} OAuth] 未找到官方 machine token 缓存，将跳过机器标识注入: {}",
            kind.log_prefix(),
    ...
    let content = fs::read_to_string(&cache_path)
        .map_err(|err| format!("读取 {} machine_token.json 失败: {}", kind.display_name(), err))?;
    let parsed = serde_json::from_str::<QoderMachineTokenCache>(&content)
        .map_err(|err| format!("解析 {} machine_token.json 失败: {}", kind.display_name(), err))?;
    ...
    logger::log_info(&format!(
        "[{} OAuth] 官方 machine token 缓存已加载: path={}, has_token={}, has_machine_type={}",
        kind.log_prefix(),
        cache_path.to_string_lossy(),
        token.is_some(),
...
fn read_qoder_cached_machine_id_for_platform(kind: QoderPlatformKind) -> Result<Option<String>, String> {
    let cache_path = default_cosy_info_path_for_platform(kind)?.join("cache").join("id");
    if !cache_path.exists() {
        logger::log_warn(&format!(
            "[{} OAuth] 未找到官方 machine id 缓存，将继续使用无机器标识链路: {}",
            kind.log_prefix(),
            cache_path.to_string_lossy()
        ));
        return Ok(None);
    ...
    let raw = fs::read_to_string(&cache_path)
        .map_err(|err| format!("读取 {} cache/id 失败: {}", kind.display_name(), err))?;
    let machine_id = normalize_non_empty(Some(raw.trim()));
    logger::log_info(&format!(
        "[{} OAuth] 官方 machine id 缓存已加载: path={}, has_machine_id={}",
        kind.log_prefix(),
        cache_path.to_string_lossy(),
        machine_id.is_some()
    ));
...
pub async fn start_login_for_platform(kind: QoderPlatformKind) -> Result<QoderOAuthStartResponse, String> {
    logger::log_info(&format!("[{} OAuth] 开始创建登录会话", kind.log_prefix()));
    let login_base_url = resolve_qoder_cli_login_endpoint(kind);
    let openapi_base_url = resolve_qoder_openapi_base_url(kind);
    let machine_info = match read_qoder_machine_info_cache_for_platform(kind) {
        Ok(value) => value,
        Err(err) => {
            logger::log_warn(&format!(
                "[{} OAuth] 读取官方 machine token 缓存失败，将继续使用无机器标识链路: {}",
                kind.log_prefix(),
                err
            ));
            None
        ...
        .and_then(|value| normalize_non_empty(Some(value.token.as_str())))
    {
        ...
        match read_qoder_cached_machine_id_for_platform(kind) {
            Ok(value) => value,
            Err(err) => {
                logger::log_warn(&format!(
                    "[{} OAuth] 读取官方 machine id 缓存失败，将继续使用无机器标识链路: {}",
                    kind.log_prefix(),
                    err
                ));
                None
    ...
    logger::log_info(&format!(
        "[{} OAuth] 已生成官方 CLI device login 链接: login_id={}, login_base_url={}, verification_uri={}, nonce_len={}, has_machine_token={}, has_machine_type={}, has_login_machine_id={}, login_machine_id_source={}",
        kind.log_prefix(),
        login_id,
    ...
    let state = PendingOAuthState {
        ...
        openapi_base_url: openapi_base_url.to_string(),
        machine_info,
    ...
    {
        let mut guard = pending_oauth_state_for_platform(kind)
            .lock()
            .map_err(|_| format!("获取 {} OAuth 状态锁失败", kind.display_name()))?;
        *guard = Some(state);
    }

    logger::log_info(&format!(
        "[{} OAuth] 登录会话已创建: login_id={}, redirect_uri={}, expires_in={}s",
        kind.log_prefix(),
        login_id,
...
pub async fn start_login() -> Result<QoderOAuthStartResponse, String> {
    start_login_for_platform(QoderPlatformKind::Global).await
}

pub async fn start_login_cn() -> Result<QoderOAuthStartResponse, String> {
    start_login_for_platform(QoderPlatformKind::Cn).await
}

pub async fn complete_login_for_platform(
    kind: QoderPlatformKind,
    login_id: &str,
) -> Result<QoderAccount, String> {
    logger::log_info(&format!(
        "[{} OAuth] 开始等待回调完成: login_id={}",
        kind.log_prefix(),
        login_id
    ));
    ...
    loop {
        let snapshot = {
            let guard = pending_oauth_state_for_platform(kind)
                .lock()
                .map_err(|_| format!("获取 {} OAuth 状态锁失败", kind.display_name()))?;
            let state = guard
                .as_ref()
                .ok_or_else(|| format!("没有进行中的 {} OAuth 登录会话", kind.display_name()))?;

            if state.login_id != login_id {
                return Err(format!("{} OAuth 登录会话已变更，请重新发起", kind.display_name()));
            }
            if state.cancelled {
                return Err(format!("{} OAuth 登录已取消", kind.display_name()));
            }
            if now_timestamp() > state.expires_at {
                clear_pending_if_matches_for_platform(kind, login_id);
                return Err(
                    last_poll_error.unwrap_or_else(|| format!("{} OAuth 登录已超时，请重试", kind.display_name()))
                );
            }
            ...
            .await
        {
            Ok(Some(token_data)) => {
                logger::log_info(&format!(
                    "[{} OAuth] deviceToken/poll 命中: login_id={}, elapsed={}ms",
                    kind.log_prefix(),
                    login_id,
                ...
                let access_token = normalize_non_empty(token_data.token.as_deref())
                    .ok_or_else(|| format!("{} device token 响应缺少 token", kind.display_name()))?;
                ...
                .await
                {
                    ...
                    Err(err) => {
                        logger::log_warn(&format!(
                            "[{} OAuth] 获取 /userinfo 失败，将继续使用 user/status: {}",
                            kind.log_prefix(),
                            err
                        ));
                        None
                ...
                .await
                {
                    ...
                    Err(err) => {
                        logger::log_warn(&format!(
                            "[{} OAuth] 获取 /api/v2/user/plan 失败，将以缺省快照继续: {}",
                            kind.log_prefix(),
                            err
                        ));
                        None
                ...
                .await
                {
                    ...
                    Err(err) => {
                        logger::log_warn(&format!(
                            "[{} OAuth] 获取 /api/v2/quota/usage 失败，将以缺省快照继续: {}",
                            kind.log_prefix(),
                            err
                        ));
                        None
                ...
                let account = qoder_account::upsert_account_from_snapshot_for_platform(
                    kind,
                    user_info_raw,
                    user_plan_raw,
                    credit_usage_raw,
                ...
                clear_pending_if_matches_for_platform(kind, login_id);
                logger::log_info(&format!(
                    "[{} OAuth] 登录完成并入库成功: login_id={}, account_id={}, email={}",
                    kind.log_prefix(),
                    login_id,
                    account.id,
                    account.email
            ...
            Err(err) => {
                ...
                logger::log_warn(&format!(
                    "[{} OAuth] deviceToken/poll 失败，等待重试: login_id={}, error={}",
                    kind.log_prefix(),
                    login_id,
                    err
                ));
        ...
        if elapsed >= next_wait_log_at {
            logger::log_info(&format!(
                "[{} OAuth] 等待 device token 中: login_id={}, elapsed={}s",
                kind.log_prefix(),
                login_id,
                elapsed.as_secs()
            ));
...
pub async fn complete_login(login_id: &str) -> Result<QoderAccount, String> {
    complete_login_for_platform(QoderPlatformKind::Global, login_id).await
}

pub async fn complete_login_cn(login_id: &str) -> Result<QoderAccount, String> {
    complete_login_for_platform(QoderPlatformKind::Cn, login_id).await
}

pub fn peek_pending_login_for_platform(kind: QoderPlatformKind) -> Option<QoderOAuthStartResponse> {
    let guard = pending_oauth_state_for_platform(kind).lock().ok()?;
...
pub fn peek_pending_login() -> Option<QoderOAuthStartResponse> {
    peek_pending_login_for_platform(QoderPlatformKind::Global)
}

pub fn peek_pending_login_cn() -> Option<QoderOAuthStartResponse> {
    peek_pending_login_for_platform(QoderPlatformKind::Cn)
}

pub fn cancel_login_for_platform(kind: QoderPlatformKind, login_id: Option<&str>) -> Result<(), String> {
    let mut guard = pending_oauth_state_for_platform(kind)
        .lock()
        .map_err(|_| format!("获取 {} OAuth 状态锁失败", kind.display_name()))?;
...
pub fn cancel_login(login_id: Option<&str>) -> Result<(), String> {
    cancel_login_for_platform(QoderPlatformKind::Global, login_id)
}

pub fn cancel_login_cn(login_id: Option<&str>) -> Result<(), String> {
    cancel_login_for_platform(QoderPlatformKind::Cn, login_id)
}
...
mod tests {
    ...
    fn builds_current_official_cosy_headers_from_machine_cache() {
        ...
        let headers = build_qoder_headers(QoderPlatformKind::Global, "access-token", Some(&machine));
