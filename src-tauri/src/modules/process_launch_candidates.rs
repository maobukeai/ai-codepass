...
pub fn should_detach_child() -> bool {
...
fn managed_proxy_env_pairs() -> Vec<(&'static str, String)> {
    ...
    .collect::<Vec<_>>()
    .join(",");
    let no_proxy = crate::modules::config::merge_local_no_proxy(&no_proxy_seed);
...
pub fn apply_managed_proxy_env_to_command(cmd: &mut Command) {
    ...
    let mut proxy_url_opt = None;
    let mut no_proxy_opt = None;
    for (key, value) in pairs {
        if key == "http_proxy" && !value.trim().is_empty() {
            proxy_url_opt = Some(value.clone());
        }
        if key == "no_proxy" && !value.trim().is_empty() {
            no_proxy_opt = Some(value.clone());
        }
        cmd.env(key, value);
    }
    ...
    if let Some(proxy_url) = proxy_url_opt {
        let prog = cmd
            ...
            .to_string_lossy()
            .to_ascii_lowercase();
        let is_shell_or_launcher = prog == "open"
            || prog == "cmd"
            || prog == "cmd.exe"
            ...
            || prog == "sh"
            || prog == "bash";
        if !is_shell_or_launcher {
            cmd.arg(format!("--proxy-server={}", proxy_url));
            if let Some(no_proxy) = no_proxy_opt {
