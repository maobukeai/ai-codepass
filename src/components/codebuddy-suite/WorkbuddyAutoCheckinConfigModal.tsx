      ...
      >
        <div className="modal-header auto-checkin-modal-header">
          <div className="auto-checkin-tabs-nav">
            ...
            >
              <Settings size={14} />
              {t('workbuddy.checkin.tabSettings', '批量自动签到设置')}
            </button>
            <button
        ...
        <div className="modal-body auto-checkin-config-body">
          {activeTab === 'settings' ? (
            <>
              <div className="config-form-item toggle-item">
                <div className="toggle-label">
                  <span className="label-title">
                    {t('workbuddy.checkin.enableAutoCheckin', '开启批量自动签到')}
                  </span>
                  <span className="label-desc">
                    {t(
                      'workbuddy.checkin.enableAutoCheckinDesc',
                      '对所有已添加账号生效。每天在指定时间窗口内为各账号随机分配时间，防风控打散自动签到',
                    )}
                  </span>
                </div>
              ...
              <div
                className="config-form-item manual-test-item"
                style={{
                  display: 'flex',
                  alignItems: 'center',
              ...
              >
                <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                  <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text-primary)' }}>
                    {t('workbuddy.autoCheckin.manualTest', '手动测试')}
                  </span>
                  <span style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>
                    {t('workbuddy.checkin.manualTestDesc', '立即触发一次后台自动签到并记录结果')}
                  </span>
                </div>
                <button
                  type="button"
                ...
                >
                  ...
                  <span>{t('workbuddy.autoCheckin.testNow', '立即执行')}</span>
                </button>
              </div>
            </>
          ) : (
