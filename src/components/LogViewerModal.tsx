  ...
  const loadSnapshot = useCallback(
    async (showLoading: boolean) => {
      try {
        ...
        setSnapshot(next);
        setError('');
        const contentStr = next.content || '';
        setRawContent(contentStr);

        const marker = clearMarkerRef.current;
        let nextVisible = contentStr;
        if (marker !== null) {
          if (contentStr === marker) {
            nextVisible = '';
          } else if (contentStr.startsWith(marker)) {
            nextVisible = contentStr.slice(marker.length).replace(/^\n+/, '');
          }
