...
function getRawPlanTag(account: QoderAccount): string | null {
  return firstNonEmptyString(
    getNestedValue(account.auth_user_plan_raw, ['name']),
    ...
    getNestedValue(account.auth_user_info_raw, ['plan']),
    getNestedValue(account.auth_user_info_raw, ['userTag']),
    getNestedValue(account.auth_user_info_raw, ['user_tag']),
    getNestedValue(account.auth_credit_usage_raw, ['planName']),
    getNestedValue(account.auth_credit_usage_raw, ['planTag']),
...
export function isSyntheticAccountEmail(email?: string | null): boolean {
  if (!email) return false;
  const lower = email.trim().toLowerCase();
  return (
    lower.startsWith('dingtalk_') ||
    lower.endsWith('@dingtalk.local') ||
    lower.endsWith('@qwenwork.local') ||
    lower.includes('@synthetic') ||
    lower.startsWith('synthetic_')
  );
}

export function getQoderAccountDisplayName(account: QoderAccount): string {
  const displayName = account.display_name?.trim();
  const email = account.email?.trim();
  const userId = account.user_id?.trim();

  if (displayName) return displayName;
  if (email && !isSyntheticAccountEmail(email)) return email;
...
export function getQoderAccountDisplayEmail(account: QoderAccount): string {
  ...
  // If email is a synthetic internal id (e.g. dingtalk_xxx@dingtalk.local), prefer displayName
  if (displayName && isSyntheticAccountEmail(email)) {
    return displayName;
  }
...
function extractValidExpiresAt(account: QoderAccount): number | null {
  const candidates: unknown[] = [
    getNestedValue(account.auth_user_info_raw, ['expireTime']),
    getNestedValue(account.auth_user_info_raw, ['expire_time']),
    getNestedValue(account.auth_user_plan_raw, ['expireTime']),
    getNestedValue(account.auth_user_plan_raw, ['expire_time']),
    getNestedValue(account.auth_user_plan_raw, ['end_date']),
    getNestedValue(account.auth_user_plan_raw, ['endDate']),
    ...
    getNestedValue(account.auth_user_info_raw, ['end_date']),
    getNestedValue(account.auth_user_info_raw, ['refreshTokenExpireTime']),
  ];

  for (const candidate of candidates) {
    const ts = normalizeTimestampMs(candidate);
    if (ts != null && ts > 1700000000000 && ts < 4000000000000) {
      return ts;
    }
  }
  return null;
  ...
  const expiresAt = extractValidExpiresAt(account);
...
export function getQoderUsage(account: QoderAccount): QoderUsage {
  ...
  const primaryQuota = hasUserQuota ? subscription.userQuota : subscription.addOnQuota;

  return {
    inlineSuggestionsUsedPercent: percent,
    chatMessagesUsedPercent: percent,
    allowanceResetAt: subscription.expiresAt,
    creditsUsed: primaryQuota.used ?? account.credits_used ?? null,
    creditsTotal: primaryQuota.total ?? account.credits_total ?? null,
    creditsRemaining: primaryQuota.remaining ?? account.credits_remaining ?? null,
  };
}
...
export function hasQoderQuotaData(account: QoderAccount): boolean {
  return (
    account.auth_credit_usage_raw != null ||
    account.credits_total != null ||
    account.auth_user_plan_raw != null
  );
}
