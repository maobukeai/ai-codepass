...
export function useSettingsPageController() {
  ...
  const [activeTab, setActiveTab] = useState<'general' | 'notifications' | 'network' | 'data' | 'about'>('general');
  ...
  const menuBarQuotaPlatformOptions: Array<{ value: PlatformId; label: string }> = [
    ...
    { value: 'trae_solo', label: 'TRAE Work' },
    { value: 'trae_cn', label: 'Trae CN' },
    { value: 'trae_solo_cn', label: 'TRAE Work CN' },
  ];
  ...
  const [menuBarQuotaPlatform, setMenuBarQuotaPlatform] = useState<PlatformId>('codebuddy');
  ...
  const [qoderCnAutoRefresh, setQoderCnAutoRefresh] = useState('10');
  ...
  const [qoderCnQuotaAlertEnabled, setQoderCnQuotaAlertEnabled] = useState(false);
  const [qoderCnQuotaAlertThreshold, setQoderCnQuotaAlertThreshold] = useState('20');
  ...
  const [qoderCnAutoRefreshCustomMode, setQoderCnAutoRefreshCustomMode] = useState(false);
  ...
  const [qoderCnQuotaAlertThresholdCustomMode, setQoderCnQuotaAlertThresholdCustomMode] = useState(false);
  ...
  const [latestUpdateStatus, setLatestUpdateStatus] = useState<{
    status: 'idle' | 'checking' | 'up_to_date' | 'has_update' | 'failed';
    version?: string;
    error?: string;
    checkedAt?: string;
  }>({ status: 'idle' });
  ...
  useEffect(() => {
    getVersion()
      .then((ver) => setAppVersion(ver ? (ver.startsWith('v') ? ver : `v${ver}`) : 'v1.0.0'))
  ...
  useEffect(() => {
    const handleStarted = (event: Event) => {
      ...
      setLatestUpdateStatus({ status: 'checking' });
    };
    ...
    const handleFinished = (event: Event) => {
      ...
      if (detail.status === 'failed') {
        ...
        setLatestUpdateStatus({
          status: 'failed',
          error: detail.error || t('settings.about.checkFailed'),
          checkedAt: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        });
        return;
      }
  ...
  useEffect(() => {
    ...
    const qoderCnAutoRefreshNum = parseInt(qoderCnAutoRefresh, 10) || -1;
    ...
    const parsedQoderCnQuotaAlertThreshold = Number.parseInt(qoderCnQuotaAlertThreshold, 10);
    ...
    const payload: Record<string, unknown> = {
      ...
      qoder_cn_auto_refresh_minutes: qoderCnAutoRefreshNum,
      ...
      claude_quota_display_remaining: claudeQuotaDisplayRemaining,
      ...
      qoder_cn_quota_alert_enabled: qoderCnQuotaAlertEnabled,
      qoder_cn_quota_alert_threshold: Number.isNaN(parsedQoderCnQuotaAlertThreshold)
        ? 20
        : parsedQoderCnQuotaAlertThreshold,
  ...
  const applyUiScale = async (rawScale: string) => {
    ...
    try {
      if (typeof window !== 'undefined' && (window as any).__TAURI_INTERNALS__?.metadata?.currentWebview) {
        await getCurrentWebview().setZoom(normalized);
      } else {
        (document.documentElement.style as any).zoom = normalized === 1 ? '' : String(normalized);
      }
    } catch {
      (document.documentElement.style as any).zoom = normalized === 1 ? '' : String(normalized);
    }
  ...
  const loadGeneralConfig = async () => {
    ...
    try {
      ...
      setMenuBarQuotaPlatform((config.menu_bar_quota_platform as PlatformId) || 'codebuddy');
      ...
      setQoderCnAutoRefresh(String(config.qoder_cn_auto_refresh_minutes ?? 10));
      ...
      setQoderCnQuotaAlertEnabled(config.qoder_cn_quota_alert_enabled ?? false);
      setQoderCnQuotaAlertThreshold(String(config.qoder_cn_quota_alert_threshold ?? 20));
  ...
  const loadNetworkConfig = async () => {
    try {
      ...
      if (config) {
        setWsEnabled(Boolean(config.ws_enabled));
        setWsPort(String(config.ws_port ?? 19528));
        setActualPort(config.actual_port ?? null);
        setDefaultPort(config.default_port ?? 19528);
        setReportEnabled(Boolean(config.report_enabled));
        setReportPort(String(config.report_port ?? 18081));
        setReportActualPort(config.report_actual_port ?? null);
        setReportDefaultPort(config.report_default_port ?? 18081);
  ...
  const loadDiagnosticsConfig = async () => {
    try {
      ...
      if (config && typeof config.errorReportingEnabled === 'boolean') {
  ...
  const isCurrentAccountRefreshAvailable = (
    ...
    switch (platform) {
      ...
      case 'qoder_cn':
        return parseRefresh(qoderCnAutoRefresh) > 0;
      case 'trae':
  ...
  const getAccountsForPlatform = (
    ...
    switch (platform) {
      ...
      case 'qoder_cn':
        return getProviderAccounts(useQoderCnAccountStore, getQoderAccountDisplayEmail);
      case 'trae':
  ...
  const qoderCnAutoRefreshIsPreset = REFRESH_PRESET_VALUES.includes(qoderCnAutoRefresh);
  ...
  const qoderCnQuotaAlertThresholdIsPreset = THRESHOLD_PRESET_VALUES.includes(qoderCnQuotaAlertThreshold);
  ...
  return {
    activeTab,
    actualPort,
    antigravityAccountGroups: [] as any[],
    antigravityAppPath,
    antigravityDualSwitchNoRestartEnabled,
    antigravityLaunchOnSwitch,
    antigravityScopeAccounts: [] as any[],
    antigravityScopeTypeOptions: [] as any[],
