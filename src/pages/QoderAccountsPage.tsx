...
import type { QoderOAuthStartResponse } from '../services/qoderService';
...
export interface QoderAccountsPageProps {
...
export function QoderAccountsPage({ platformId = 'qoder' }: QoderAccountsPageProps = {}) {
  ...
  const filterPersistenceScope = normalizeAccountsOverviewScope(platformId);
  const flowNoticeCollapsedKey = isQwenwork
    ? 'agtools.qwenwork.flow_notice_collapsed'
    : isCn
    ? 'agtools.qoder_cn.flow_notice_collapsed'
    : 'agtools.qoder.flow_notice_collapsed';
  const exportFilePrefix = isQwenwork
    ? 'qwenwork_accounts'
    : isCn
    ? 'qoder_cn_accounts'
    : 'qoder_accounts';

  const globalStore = useQoderAccountStore();
  const cnStore = useQoderCnAccountStore();
  const qwenworkStore = useQwenworkAccountStore();
  ...
  const [isFlowNoticeCollapsed, setIsFlowNoticeCollapsed] = useState<boolean>(() =>
    readBooleanStorage(flowNoticeCollapsedKey, true),
  );
  ...
  }, [oauthLoginId, qoderService, resetOauthState, showAddModal]);
  ...
  const pagination = usePagination({
    ...
    storageKey: buildPaginationPageSizeStorageKey(isCn ? 'QoderCn' : 'Qoder'),
  });
  ...
  const handleDeleteAccounts = useCallback(
    async (ids: string[]) => {
      ...
      const confirmed = await confirmDialog(
        ...
        {
          title: t('common.appName', 'AI CodePass'),
          kind: 'warning',
  ...
  const handleImportLocal = useCallback(async () => {
    ...
    try {
      ...
      setAddMessage(
        isCn
          ? t('qoder.import.localSuccessCn', '已从本机 Qoder 国内版导入账号。')
          : t('qoder.import.localSuccess', '已从本机 Qoder 导入账号。'),
      );
      window.setTimeout(() => {
        setShowAddModal(false);
      }, 600);
  ...
  }, [addStatus, isCn, qoderService, store, t]);
  ...
  const handlePrepareOauth = useCallback(async () => {
    ...
    try {
        ...
        void qoderService
          ...
          .then(async (completedAccount) => {
            ...
            for (let attempt = 0; attempt < 3; attempt += 1) {
              await store.fetchAccounts();
              const state = isCn ? useQoderCnAccountStore.getState() : useQoderAccountStore.getState();
      ...
      let response: QoderOAuthStartResponse;
      try {
        ...
        let peeked: QoderOAuthStartResponse | null = null;
      ...
      void openUrl(verificationUri).catch(() => {
        try {
          window.open(verificationUri, '_blank', 'noopener,noreferrer');
        } catch {
          // ignore browser popup blocker
        }
      });
    } catch (error) {
      if (attemptSeq !== oauthAttemptSeqRef.current) return;
  ...
  }, [isCn, oauthCompleting, oauthLoginId, oauthPreparing, qoderService, store, t]);
  ...
  const handleOpenOauthUrl = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        logQoderOauthUi('link:open-browser-failed', { error: String(fallbackError) });
  ...
  }, [addTab, oauthLoginId, oauthPreparing, oauthUrl, qoderService, showAddModal]);
  ...
  const handleExportSelected = useCallback(async () => {
    ...
    await handleExportByIds(ids, exportFilePrefix);
  }, [exportFilePrefix, filteredIds, handleExportByIds, selected]);
  ...
  const resolveUpdatedText = useCallback(
    (account: QoderAccount) => {
      const updatedAt = account.usage_updated_at || account.last_used || account.created_at || Math.floor(Date.now() / 1000);
  ...
  const resolveQuotaDisplay = useCallback(
    (account: QoderAccount): QoderQuotaDisplay => {
      ...
      const buildQuotaItem = (
        ...
        const valueText =
          normalizedTotal > 0
            ? `剩余 ${formatQuotaValue(remaining)} / 总量 ${formatQuotaValue(normalizedTotal)}`
            : '0 / 0';
        ...
        return {
          ...
          percentageText: `已用 ${normalizedPercent}%`,
          valueText,
          showProgress: true,
        };
      };
      ...
      if (hasAddonQuota && !hasUserQuota) {
        items.push(
          buildQuotaItem(
            'creditPackage',
            isQwenwork
              ? t('qwenwork.quota.longtermCredits', '长期/活动 Credits')
        ...
        items.push(
          buildQuotaItem(
            'included',
            isQwenwork
              ? t('qwenwork.quota.dailyCredits', '每日刷新 Credits')
      ...
      if (
        (subscription.sharedCreditPackageUsed != null && subscription.sharedCreditPackageUsed > 0) ||
        subscription.isPersonalVersion === false
      ) {
        items.push({
          key: 'sharedCreditPackage' as const,
          label: t('common.shared.columns.sharedCreditPackage', '共享资源包'),
          normalizedPercent: 0,
          quotaClass: 'high' as const,
          percentageText: null,
      ...
      return {
        ...
        resetText: (() => {
          if (isQwenwork) {
            const hasAddon = (subscription.addOnQuota.total ?? 0) > 0;
            if (resetAt != null && hasAddon) {
              const diffDays = Math.ceil((resetAt - Date.now()) / (86400 * 1000));
              const countdown =
                diffDays > 0
                  ? diffDays === 1
                    ? ' (明天到期)'
                  ...
                  : diffDays === 0
                    ? ' (今日到期)'
                    : ' (已到期)';
              return `长期积分到期：${formatDisplayDate(resetAt)}${countdown} (每日 00:00 刷新)`;
            }
            return '每日额度将于 00:00 自动刷新';
          }
          ...
          const countdown =
            diffDays > 0
              ...
              : diffDays === 0
                ? ' (今日到期)'
                : ' (已到期)';
          return t('qoder.quota.expiresAt', {
            date: `${formatDisplayDate(resetAt)}${countdown}`,
            defaultValue: `到期时间：${formatDisplayDate(resetAt)}${countdown}`,
          });
        })(),
      };
    },
  ...
  const platformDisplayName = isQwenwork ? '千问办公' : isCn ? 'Qoder 国内版' : 'Qoder 国际版';

  const handleQoderCheckin = useCallback(
    async (accountId?: string) => {
      if (checkingIn) return;
      setCheckingIn(true);
      try {
        const res = await qoderService.claimQoderCheckin(accountId ?? currentAccountId ?? undefined);
        await store.fetchAccounts();
        if (res.success === false) {
          setMessage({
            tone: 'error',
            text: res.message || `${platformDisplayName} 当前暂无可申领的签到/福利活动`,
          });
          return;
        }
        if (res.alreadyCheckedIn) {
          setMessage({
            ...
            text:
              res.message ||
              (isQwenwork
                ? '千问办公今日已完成签到核验，每日 100 Credits 额度已生效（每日 00:00 自动刷新）'
                : `${platformDisplayName} 今日已领取过签到奖励，明日再来吧`),
          });
          return;
        }
        const successMsg =
          res.message ||
          (isQwenwork
            ? '千问办公每日签到成功！已核实并激活今日 100 Credits 算力额度（每日 00:00 自动刷新）'
            : `${platformDisplayName} 每日签到成功，获得 +100 Credits！`);
        setMessage({
          tone: 'success',
          text: successMsg,
        });
        try {
          if ('Notification' in window && Notification.permission === 'granted') {
            new Notification(`${platformDisplayName}签到通知`, {
              body: successMsg,
            });
          }
        } catch {
        ...
        setMessage({
          tone: 'error',
          text: `${platformDisplayName}签到失败：${err instanceof Error ? err.message : String(err)}`,
        });
      } finally {
        setCheckingIn(false);
      }
    },
    [checkingIn, currentAccountId, platformDisplayName, qoderService, store],
  );

  const renderGridCards = useCallback(
    (items: QoderAccount[], groupKey?: string) =>
      items.map((account) => {
        const displayName = getQoderAccountDisplayName(account);
        const maskedEmail = maskAccountText(displayName);
        ...
        return (
          <div
            ...
            className={`ghcp-account-card apple-account-card ${isCurrent ? 'current' : ''} ${isSelected ? 'selected' : ''}`}
          >
            <div className="card-top">
              ...
              <div className="account-avatar-squircle">
                {isQwenwork ? (
                  <QwenWorkIcon style={{ width: 22, height: 22 }} />
                ) : (
                  <QoderIcon style={{ width: 22, height: 22 }} />
                )}
                {isCurrent && <span className="avatar-status-dot active" title={t('accounts.status.configuredTip', '此账号为当前写入本地软件的配置账号')} />}
              </div>
              <div className="account-title-wrap">
                <div className="account-main-line">
                  ...
                  {isCurrent && (
                    <span className="current-tag" title={t('accounts.status.configuredTip', '此账号为当前写入本地软件的配置账号')}>
                      <span className="current-tag-dot" />
                      {t('accounts.status.configured', '当前配置')}
                    </span>
                  )}
                  {quotaError && (
                    <span className="status-pill warning" title={quotaError}>
                      <CircleAlert size={12} />
                      {t('common.shared.quota.queryFailed', '配额失败')}
                    </span>
                  )}
                </div>
                <div className="account-sub-line">
                  <span className="account-meta-text">
                    {isQwenwork ? '千问办公 (阿里)' : isCn ? 'Qoder 国内版' : 'Qoder 阿里'} · ID: {maskAccountText(account.user_id || account.id)}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="card-footer">
              <span className="card-date" title={createdAtText}>
                <CalendarCheck size={12} style={{ marginRight: 3, opacity: 0.7 }} />
                {updatedText}
              </span>
              <div className="card-actions">
                <button
                  className={`card-action-btn switch-btn ${isCurrent ? 'active-current' : 'primary-switch'}`}
                  onClick={() => void handleSwitch(account.id)}
                  disabled={isInjecting || deleting}
                  title={isCurrent ? t('accounts.status.configuredActionTip', '当前已写入本地配置，点击可重新切号应用并启动软件') : t('dashboard.switch', '切换账号')}
                >
                  {isInjecting ? (
                ...
                <button
                  ...
                  onClick={() => void handleQoderCheckin(account.id)}
                  disabled={checkingIn || isInjecting || deleting}
                  title={`${platformDisplayName}每日签到 (+100 Credits)`}
                >
                  <CalendarCheck size={13} className={checkingIn ? 'loading-spinner' : ''} />
                </button>
                <button
                  className="card-action-btn secondary"
                ...
                >
                  <RefreshCw size={13} className={isRefreshing ? 'loading-spinner' : ''} />
                </button>
                <button
                  className="card-action-btn secondary export-btn"
                ...
                <button
                  className="card-action-btn secondary danger"
      ...
      items.map((account) => {
        ...
        return (
          <tr key={groupKey ? `${groupKey}-${account.id}` : account.id} className={isCurrent ? 'current' : undefined}>
            ...
            <td title={maskAccountText(getQoderAccountDisplayName(account))}>
              <div className="account-cell">
                <div className="account-main-line">
                  {maskAccountText(getQoderAccountDisplayName(account))}
                </div>
                {quotaError && (
                  <div className="account-sub-line">
                    <span className="status-pill warning" title={quotaError}>
  ...
  return (
    <div className="ghcp-accounts-page qoder-accounts-page">
      <PlatformOverviewTabsHeader platform={platformId} active={activeTab} onTabChange={setActiveTab} />

      {activeTab === 'instances' ? (
        <QoderInstancesContent platformId={platformId} accountsForSelect={filteredAccounts} />
      ) : (
        <>
          <div className={`ghcp-flow-notice ${isFlowNoticeCollapsed ? 'collapsed' : ''}`} role="note" aria-live="polite">
            <button
              ...
              onClick={() =>
                setIsFlowNoticeCollapsed((prev) => {
                  const next = !prev;
                  writeBooleanStorage(flowNoticeCollapsedKey, next);
                  return next;
                })
              }
            ...
            >
              <div className="ghcp-flow-notice-title">
                ...
                <span>
                  {isQwenwork
                    ? t('qwenwork.flowNotice.title', '千问办公账号接入说明（点击展开/收起）')
                    : isCn
                    ? t('qoderCn.flowNotice.title', 'Qoder 国内版账号接入说明（点击展开/收起）')
                    : t('qoder.flowNotice.title', 'Qoder 账号接入说明（点击展开/收起）')}
                </span>
              </div>
            ...
            {!isFlowNoticeCollapsed && (
              <div className="ghcp-flow-notice-body">
                <div className="ghcp-flow-notice-desc">
                  {isQwenwork
                    ? t(
                        'qwenwork.flowNotice.desc',
                        '当前支持本机千问办公（QwenWork CN）加密存储解密导入、JSON 导入、一键切号注入、每日签到领取 Credits、应用多开绑定与配额概览。',
                      )
                    : isCn
                    ? t(
                        'qoderCn.flowNotice.desc',
                        '当前支持官方授权登录（回调）、本地导入、JSON 导入、切号注入、应用多开绑定与配额概览。登录流程沿用 Qoder 国内版客户端真实落盘数据。',
                      )
                    : t(
                ...
                <ul className="ghcp-flow-notice-list">
                  <li>
                    {isQwenwork
                      ? t(
                          'qwenwork.flowNotice.permission',
                          '权限范围：读取千问办公本地认证存储（%APPDATA%\\QwenWorkCN\\auth-v2.dat 与 ~/.qwenworkcn/.status.json），用于账号切换与每日签到；所有数据仅在本机处理。',
                        )
                      : isCn
                      ? t(
                          'qoderCn.flowNotice.permission',
                          '权限范围：读取 Qoder 国内版本地认证存储（auth 凭据与用户信息），用于账号切换与会话注入；所有数据仅在本机处理。',
                        )
                      : t(
                          'qoder.flowNotice.permission',
                          '权限范围：读取 Qoder 本地认证存储（auth 凭据与用户信息），用于账号切换与会话注入；所有数据仅在本机处理。',
                        )}
                  </li>
                  <li>
                    {isQwenwork
                      ? t(
                          'qwenwork.flowNotice.network',
                          '网络范围：每日签到与配额查询通过千问办公官方网关（gateway.qwenwork.cn）执行，不上传本地密钥或凭证。',
                        )
                      : isCn
                      ? t(
                          'qoderCn.flowNotice.network',
                          '网络范围：OAuth 授权登录需联网请求 Qoder 国内版官方服务（qoder.cn）完成回调；配额查询通过 Qoder API 获取用量数据。不上传本地密钥或凭证。',
                        )
                      : t(
                          'qoder.flowNotice.network',
                          '网络范围：OAuth 授权登录需联网请求 Qoder 官方服务完成回调；配额查询通过 Qoder API 获取用量数据。不上传本地密钥或凭证。',
                        )}
                  </li>
                </ul>
              </div>
          ...
          <div className="toolbar">
            ...
            <div className="toolbar-right">
              ...
              <QuickSettingsPopover type={platformId === 'qwenwork' ? 'qoder_cn' : platformId} />
            </div>
          </div>
          ...
          {loading && accounts.length === 0 ? (
            ...
            <div className="empty-state">
              ...
              <p>
                {isCn
                  ? t('qoderCn.empty.desc', '点击“添加账号”，可使用授权登录、本机导入或 JSON 导入。')
                  : t('qoder.empty.desc', '点击“添加账号”，可使用授权登录、本机导入或 JSON 导入。')}
              </p>
              <button
      ...
      {showAddModal && (
        <div className="modal-overlay">
          <div className="modal modal-content codex-add-modal platform-account-add-modal" onClick={(event) => event.stopPropagation()}>
            <div className="modal-header">
              ...
              <h2>{isCn ? t('qoderCn.addModal.title', '添加 Qoder 国内版账号') : t('qoder.addModal.title', '添加 Qoder 账号')}</h2>
              ...
              {addTab === 'oauth' ? (
                <div className="add-section">
                  <p className="section-desc">
                    {isCn
                      ? t('qoderCn.oauth.hint', '点击下方按钮，在浏览器中完成 Qoder 国内版账号 OAuth 授权。')
                ...
                <div className="add-section">
                  <p className="section-desc">
                    {isCn
                      ? t('accounts.importJsonHintCn', '导入由本工具导出的 Qoder 国内版 JSON 文件。')
                      : t('accounts.importJsonHint', '导入由本工具导出的 Qoder JSON 文件。')}
                  </p>
                  <textarea
                ...
                <div className="add-section">
                  <p className="section-desc">
                    {isCn
                      ? t('qoderCn.import.localDesc', '从本地已安装的 Qoder 国内版客户端读取认证信息并导入。')
                      : t('qoder.import.localDesc', '从本地已安装的 Qoder 客户端读取认证信息并导入。')}
                  </p>
