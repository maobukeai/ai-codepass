  ...
  const resolveQuotaSummary = useCallback(
    (account: TraeAccount): TraeQuotaSummary => {
      ...
      const hasCredits =
        ...
        (usage.creditsTotal > 0 || (usage.creditsUsed ?? 0) > 0);
      ...
      let costText: string;
      if (hasCredits) {
        costText = t('trae.quota.creditsUsedOfTotal', {
          used: formatNumber(usage.creditsUsed ?? 0),
          total: formatNumber(usage.creditsTotal ?? 0),
      ...
      const statusText = !hasUsageRaw
        ...
        : hasCredits
          ? usage.usageExhausted
            ? t('trae.quota.statusExhaustedFree', '状态：用量已耗尽，建议升级方案')
            : t('trae.quota.creditsRemainingStatus', {
                available: formatNumber(usage.creditsAvailable ?? 0),
                defaultValue: '状态：正常 · 剩余 {{available}} 积分',
              })
          : isCnAccount
            ? hasFastRequest
              ? usage.usageExhausted
      ...
      const bonusText = hasCredits
        ? (usage.creditsPackCount ?? 0) > 0
          ? t('trae.quota.creditsPackCount', {
              count: usage.creditsPackCount ?? 0,
              defaultValue: '积分包：{{count}} 个有效',
            })
          : ''
        : isCnAccount
          ? hasFastRequest
      ...
      return {
        ...
        percentageText: effectivePercent == null ? '--' : `${effectivePercent}%`,
        quotaClass: computeQuotaClass(effectivePercent),
        costText,
        statusText,
        statusTone,
        bonusText,
        resetText: (() => {
          const resetVal = usage.resetAt ?? account.plan_reset_at ?? null;
          if (resetVal == null) return t('trae.quota.resetUnknown', '重置时间未知');
          const resetMs = resetVal >= 1_000_000_000_000 ? resetVal : resetVal * 1000;
          const diffDays = Math.ceil((resetMs - Date.now()) / (86400 * 1000));
          const countdown = diffDays > 0 ? (diffDays === 1 ? ' (明天)' : ` (${diffDays}天后)`) : diffDays === 0 ? ' (今日)' : ' (已到期)';
          return t('trae.quota.resetAt', {
            date: `${formatTraeResetAt(resetVal)}${countdown}`,
            defaultValue: `重置时间：${formatTraeResetAt(resetVal)}${countdown}`,
          });
        })(),
        packageText,
        ...
        <div className={`quota-item trae-compact-quota ${variant === 'table' ? 'is-table windsurf-table-credit-item' : 'is-card'}`}>
          <div className="quota-header trae-compact-quota-header">
            ...
            <div className="trae-compact-quota-main">
              ...
              {quota.percentageText && quota.percentageText !== '--' && (
          ...
          <div className="trae-compact-quota-meta-row">
            {quota.statusText && (
              <div className={`trae-compact-quota-status ${quota.statusTone}`} title={quota.statusText}>
                {quota.statusText}
              </div>
            )}
            {quota.resetText && (
              <div className="trae-compact-quota-reset" title={quota.resetText}>
                {quota.resetText}
              </div>
            )}
          </div>
          ...
          >
            <div className="card-top">
              ...
              <div className="account-avatar-squircle">
                {renderPlatformIcon(platformId, 22)}
              ...
              <div className="account-title-wrap">
                <div className="account-main-line">
                  ...
                  {hasStatusError && (
                    <span className="status-pill warning" title={statusTitle}>
                      <CircleAlert size={12} />
                      {t('accounts.status.refreshFailed', '异常')}
                    </span>
                  )}
                  {quotaError && (
                ...
                <div className="account-sub-line">
                  <span className="account-meta-text">
                    {signedInWithText} · ID: {maskAccountText(userIdText)}
                    {showDisplayEmail && ` · ${maskAccountText(displayEmail)}`}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="ghcp-quota-section">
              {hasQuotaData ? (
                ...
                <div className="quota-empty-pill">
                  <span className="quota-dot" />
                  <span>{t('common.shared.quota.readyOrUnlimited', '账号已就绪 · 配额正常')}</span>
                </div>
              )}
            </div>
            ...
            <div className="card-footer">
              ...
              <div className="card-actions">
                <button
                  ...
                  title={isCurrent ? t('accounts.status.configuredActionTip', '当前已写入本地配置，点击可重新切号应用并启动软件') : t('dashboard.switch', '一键切换此账号')}
                >
          ...
          <tr key={groupKey ? `${groupKey}-${account.id}` : account.id} className={isCurrent ? 'current' : ''}>
            ...
            <td>
              <div className="account-cell">
                <div className="account-main-line">
                  ...
                  {isCurrent && (
                    <span className="mini-tag current" title={t('accounts.status.configuredTip', '此账号为当前写入本地软件的配置账号')}>{t('accounts.status.configured', '当前配置')}</span>
                  )}
                </div>
                {hasStatusError && (
                  <div className="account-sub-line">
            ...
            <td>
              {hasQuotaData ? (
                ...
                <div className="quota-empty-pill">
                  <span className="quota-dot" />
                  <span>{t('common.shared.quota.readyOrUnlimited', '账号已就绪')}</span>
                </div>
              )}
            </td>
            <td>
            ...
            <td className="sticky-action-cell table-action-cell">
              <div className="card-actions">
                <button
                  ...
                  onClick={() => handleInjectToVSCode?.(account.id)}
                  disabled={injecting === account.id}
                  title={isCurrent ? t('accounts.status.configuredActionTip', '当前已写入本地配置，点击可重新切号应用并启动软件') : t('dashboard.switch', '切换')}
                >
                  {injecting === account.id ? (
                    <RefreshCw size={13} className="loading-spinner" />
                  ) : isCurrent ? (
              ...
              <button
                ...
                title={
                  platformId === 'trae_cn' || platformId === 'trae_solo_cn'
                    ? t('trae.checkin.modalTitle', '每日签到领额度')
                    : t(
                        'trae.checkin.globalNotSupportedTooltip',
                        '国际版官方无签到活动（为月度自动重置模式，国内版支持每日签到）',
                      )
                }
                aria-label={t('trae.checkin.modalTitle', '每日签到领额度')}
              >
        ...
        <div className="grid-view-container">
          ...
          {showAddModal && (
            <div className="modal-overlay">
              <div className="modal modal-content ghcp-add-modal platform-account-add-modal" onClick={(event) => event.stopPropagation()}>
