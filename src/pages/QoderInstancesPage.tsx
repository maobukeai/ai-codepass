...
import { useQwenworkInstanceStore } from '../stores/useQwenworkInstanceStore';
...
export type QoderPlatformId = 'qoder' | 'qoder_cn' | 'qwenwork';
...
export function QoderInstancesContent({
  ...
  const globalInstanceStore = useQoderInstanceStore();
  const cnInstanceStore = useQoderCnInstanceStore();
  const qwenworkInstanceStore = useQwenworkInstanceStore();
  const instanceStore =
    platformId === 'qwenwork'
      ? qwenworkInstanceStore
      : platformId === 'qoder_cn'
      ? cnInstanceStore
      : globalInstanceStore;

  const globalAccountStore = useQoderAccountStore();
  const cnAccountStore = useQoderCnAccountStore();
  const qwenworkAccountStore = useQwenworkAccountStore();
  const accountStore =
    platformId === 'qwenwork'
      ? qwenworkAccountStore
      : platformId === 'qoder_cn'
      ? cnAccountStore
      : globalAccountStore;
  ...
  return (
    <PlatformInstancesContent<QoderAccount>
      ...
      appType={platformId === 'qwenwork' ? 'qoder_cn' : platformId}
      isSupported={isSupportedPlatform}
      unsupportedTitleKey="common.shared.instances.unsupported.title"
      unsupportedTitleDefault="暂不支持当前系统"
      unsupportedDescKey={
        platformId === 'qwenwork'
          ? 'qwenwork.instances.unsupported.descPlatform'
          : platformId === 'qoder_cn'
          ? 'qoderCn.instances.unsupported.descPlatform'
          : 'qoder.instances.unsupported.descPlatform'
      }
      unsupportedDescDefault={
        platformId === 'qwenwork'
          ? '千问办公应用多开仅支持 macOS、Windows 和 Linux。'
          : platformId === 'qoder_cn'
          ? 'Qoder 国内版应用多开仅支持 macOS、Windows 和 Linux。'
          : 'Qoder 应用多开仅支持 macOS、Windows 和 Linux。'
      }
    />
  );
}
