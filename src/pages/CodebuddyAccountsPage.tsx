...
import {
  ...
  Tag, Play, Eye, EyeOff, CircleAlert, ChevronDown, CalendarCheck,
} from 'lucide-react';
        ...
        <div key={groupKey ? `${groupKey}-${account.id}` : account.id}
          ...
          <div className="card-top">
            ...
            <div className="account-avatar-squircle">
              <CodebuddyIcon style={{ width: 22, height: 22 }} />
            ...
            <div className="account-title-wrap">
              ...
              <div className="account-sub-line">
                <span className="account-meta-text">
                  CodeBuddy · {account.nickname || '腾讯工蜂'}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="card-footer">
            ...
            <div className="card-actions">
              <button
                ...
                disabled={!!injecting}
                title={isCurrent ? t('accounts.status.configuredActionTip', '当前已写入本地配置，点击可重新切号应用并启动软件') : t('common.shared.switchAccount', '切换账号')}
              >
              ...
              <button className="card-action-btn secondary" onClick={() => openTagModal(account.id)} title={t('accounts.editTags', '编辑标签')}><Tag size={13} /></button>
              <button className="card-action-btn secondary" onClick={() => handleRefresh(account.id)} disabled={refreshing === account.id} title={t('common.shared.refreshQuota', '刷新')}>
                <RotateCw size={13} className={refreshing === account.id ? 'loading-spinner' : ''} />
              </button>
              <button className="card-action-btn secondary export-btn" onClick={() => handleExportByIds([account.id])} title={t('common.shared.export.title', '导出')}><Upload size={13} /></button>
              <button className="card-action-btn secondary danger" onClick={() => handleDelete(account.id)} title={t('common.delete', '删除')}><Trash2 size={13} /></button>
            </div>
          </div>
        </div>
      );
    ...
    items.map((account) => {
      ...
      return (
        <tr key={account.id} className={`${isCurrent ? 'current-row' : ''} ${isSelected ? 'selected-row' : ''}`}>
          ...
          <td>
            <div className="account-main-line">
              <span className="account-email-text" title={maskAccountText(displayEmail)}>{maskAccountText(displayEmail)}</span>
              {isCurrent && <span className="mini-tag current" title={t('accounts.status.configuredTip', '此账号为当前写入本地软件的配置账号')}>{t('accounts.status.configured', '当前配置')}</span>}
            </div>
          </td>
          ...
          <td className="sticky-action-cell table-action-cell">
            <div className="card-actions">
              <button
                ...
                onClick={() => handleInjectToVSCode?.(account.id)}
                disabled={!!injecting}
                title={isCurrent ? t('accounts.status.configuredActionTip', '当前已写入本地配置，点击可重新切号应用并启动软件') : t('common.shared.switchAccount', '切换')}
              >
                {injecting === account.id ? <RefreshCw size={13} className="loading-spinner" /> : isCurrent ? <><Check size={12} /><span>已生效</span></> : <><Play size={12} /><span>切号</span></>}
              </button>
              <button className="card-action-btn secondary" onClick={() => openTagModal(account.id)} title={t('accounts.editTags', '编辑标签')}><Tag size={13} /></button>
              <button className="card-action-btn secondary" onClick={() => handleRefresh(account.id)} disabled={refreshing === account.id} title={t('common.shared.refreshQuota', '刷新')}><RotateCw size={13} className={refreshing === account.id ? 'loading-spinner' : ''} /></button>
              <button className="card-action-btn secondary" onClick={() => handleExportByIds([account.id])} title={t('common.shared.export.title', '导出')}><Upload size={13} /></button>
              <button className="card-action-btn secondary danger" onClick={() => handleDelete(account.id)} title={t('common.delete', '删除')}><Trash2 size={13} /></button>
            </div>
          </td>
        </tr>
    ...
    <div className="ghcp-accounts-page codebuddy-accounts-page">
      ...
      {showAddModal && (
        <div className="modal-overlay">
            <div className="modal modal-content ghcp-add-modal platform-account-add-modal" onClick={(e) => e.stopPropagation()}>
