...
export function getLocalCheckinDates(accountId: string): string[] {
  ...
  try {
    const raw = localStorage.getItem(`agtools.qoder_cn.checkin_dates.${accountId}`);
...
export function saveLocalCheckinDate(accountId: string, dateStr: string): void {
  ...
  try {
    ...
    if (!existing.includes(dateStr)) {
      ...
      localStorage.setItem(`agtools.qoder_cn.checkin_dates.${accountId}`, JSON.stringify(updated));
    }
  } catch {
    // ignore
  }
...
export async function getQoderCheckinStatus(accountId: string): Promise<{
  ...
  try {
    const res = await invoke<{
      ...
      checkin_dates?: string[] | null;
      message?: string;
    }>('get_qoder_cn_checkin_status', { accountId });
  ...
  return {
    ...
    today_credit: 100,
    checkin_dates: mergedDates,
    message: isCheckedIn ? 'Qoder 国内版今日已完成签到核验' : 'Qoder 国内版待签到领取 +100 Credits',
  };
}
