...
const NEVER_REMOTE_HIDE_PLATFORM_IDS = new Set<PlatformId>();
...
function normalizeRemoteConfigState(raw: RemoteConfigStateRaw | null | undefined): RemoteConfigState {
  if (!raw) {
    return {
      version: '',
...
export async function getRemoteConfigState(): Promise<RemoteConfigState> {
  try {
    ...
    return normalizeRemoteConfigState(null);
  }
}
