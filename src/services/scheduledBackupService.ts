...
export function getSelectionFromAutoBackupSettings(
  settings?: Pick<AutoBackupSettings, 'include_accounts' | 'include_config'> | null,
...
const DEFAULT_BACKUP_SETTINGS: AutoBackupSettings = {
  enabled: true,
...
export async function getAutoBackupSettings(): Promise<AutoBackupSettings> {
  try {
    const res = await invoke<AutoBackupSettings>('get_auto_backup_settings');
    return res || DEFAULT_BACKUP_SETTINGS;
  } catch {
...
export async function getBackupUsage(): Promise<BackupUsageSummary> {
  try {
    const res = await invoke<BackupUsageSummary>('get_backup_usage');
    return res || { total_file_count: 0, total_size_bytes: 0, entries: [] };
  } catch {
    return { total_file_count: 0, total_size_bytes: 0, entries: [] };
  }
}
...
export async function listAutoBackupFiles(): Promise<AutoBackupFileEntry[]> {
  try {
    const res = await invoke<AutoBackupFileEntry[]>('list_auto_backup_files');
...
export function isAutoBackupDue(settings?: AutoBackupSettings | null, now = new Date()): boolean {
