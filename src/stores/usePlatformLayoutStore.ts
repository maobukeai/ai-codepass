...
export const CODEBUDDY_SUITE_PLATFORM_IDS: PlatformId[] = ['codebuddy', 'codebuddy_cn', 'workbuddy', 'workbuddy_ai'];
...
export const QODER_SUITE_PLATFORM_IDS: PlatformId[] = ['qoder', 'qoder_cn', 'qwenwork'];
...
interface PlatformLayoutState {
  ...
  rememberGroupPlatform: (platformId: PlatformId) => void;
...
export function resolveGroupChildName(
  ...
  if (platformId === 'trae_solo' && name === 'TRAE SOLO') {
    return fallbackName;
  }
  if (platformId === 'trae_solo_cn' && name === 'TRAE SOLO CN') {
    return fallbackName;
  }
  return name;
}
...
function createDefaultTraeSuiteGroup(): PlatformLayoutGroup {
  return {
    ...
    childConfigs: [
      ...
      { platformId: 'trae_solo', name: 'TRAE Work' },
      { platformId: 'trae_cn', name: 'Trae CN' },
      { platformId: 'trae_solo_cn', name: 'TRAE Work CN' },
    ],
  };
}
...
function createDefaultQoderSuiteGroup(): PlatformLayoutGroup {
  return {
    ...
    childConfigs: [
      { platformId: 'qoder', name: 'Qoder' },
      { platformId: 'qoder_cn', name: 'Qoder 国内版' },
      { platformId: 'qwenwork', name: '千问办公' },
    ],
  };
}
...
function defaultSidebarPlatformIds(): PlatformId[] {
  return ['codebuddy', 'codebuddy_cn', 'workbuddy', 'qoder', 'qoder_cn', 'trae', 'trae_solo', 'trae_cn', 'trae_solo_cn'];
}
...
function normalizeTray(
  tray: PlatformId[],
  _rawOrder: PlatformId[] = [],
  _allowLegacyMigration = false,
): PlatformId[] {
...
function normalizeGroupName(raw: unknown, fallbackPlatform: PlatformId): string {
  ...
  if (fallbackPlatform === 'qoder_cn') {
    return 'Qoder 国内版';
  }
  if (fallbackPlatform === 'qwenwork') {
    return '千问办公';
  }
...
function normalizeGroupChildName(raw: unknown, platformId: PlatformId): string | undefined {
  ...
  if (platformId === 'trae_solo' && value === 'TRAE SOLO') {
    value = 'TRAE Work';
  }
  if (platformId === 'trae_solo_cn' && value === 'TRAE SOLO CN') {
    value = 'TRAE Work CN';
  }
  return value;
}
...
function normalizePlatformGroups(
  ...
  if (missingQoderSuiteIds.length > 0) {
    const qoderGroup =
    ...
    if (qoderGroup) {
      ...
      if (qoderGroup.name === 'platform-qoder' || qoderGroup.name === 'Qoder') {
        qoderGroup.name = 'Qoder';
      }
  ...
  const missingCodebuddySuiteIds = CODEBUDDY_SUITE_PLATFORM_IDS.filter(
    (platformId) => !usedPlatformIds.has(platformId),
  );
  if (missingCodebuddySuiteIds.length > 0) {
    const cbGroup =
      result.find((group) => group.id === DEFAULT_CODEBUDDY_GROUP_ID)
      ?? result.find((group) => group.platformIds.includes('codebuddy'));
    if (cbGroup) {
      cbGroup.platformIds = [...cbGroup.platformIds, ...missingCodebuddySuiteIds];
      if (!CODEBUDDY_SUITE_PLATFORM_IDS.includes(cbGroup.defaultPlatformId)) {
        cbGroup.defaultPlatformId = 'codebuddy';
      }
      if (cbGroup.iconKind !== 'custom') {
        cbGroup.iconPlatformId = 'codebuddy';
      }
      cbGroup.childConfigs = normalizeGroupChildConfigs(
        [
          ...(cbGroup.childConfigs ?? []),
          { platformId: 'codebuddy', name: 'CodeBuddy' },
          { platformId: 'codebuddy_cn', name: 'CodeBuddy CN' },
          { platformId: 'workbuddy', name: 'WorkBuddy' },
          { platformId: 'workbuddy_ai', name: 'WorkBuddy AI' },
        ],
        cbGroup.platformIds,
      );
      missingCodebuddySuiteIds.forEach((platformId) => usedPlatformIds.add(platformId));
    }
  }
...
export const usePlatformLayoutStore = create<PlatformLayoutState>((set, get) => ({
  ...
  rememberGroupPlatform: (platformId) => {
    const currentGroups = get().platformGroups;
    const group = currentGroups.find((item) => item.platformIds.includes(platformId));
    if (!group || group.defaultPlatformId === platformId) {
      return;
    }

    const nextGroups = currentGroups.map((item) =>
      item.id === group.id
        ? {
            ...item,
            defaultPlatformId: platformId,
