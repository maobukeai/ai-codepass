...
export function useProviderAccountsPage<TAccount extends ProviderAccountBase>(
    ...
    (request: ExternalProviderImportPayload) => {
      ...
      void (async () => {
        try {
          ...
          const items = applyCockpitApiBaseUrlToExternalImportItems(resolvedItems, request);
  ...
  const prepareOauthUrl = useCallback(() => {
    const service = oauthServiceRef.current;
    if (!service) return;
    ...
    void (async () => {
      try {
        const resp = await service.startLogin({ tabKey: addTabRef.current });
        started = true;

        if (attemptSeq !== oauthAttemptSeqRef.current) {
          service.cancelLogin(resp.loginId).catch(() => {});
        ...
        // 自动拉起默认浏览器进行授权登录，若拉起失败亦不影响界面手动点击打开
        if (url) {
          try {
            if (service.openAuthUrl) {
              await service.openAuthUrl(url, false);
            } else {
              await openUrl(url);
            }
          } catch (openErr) {
            console.warn(`[${oauthLogPrefix}] 自动打开浏览器失败，用户仍可手动点击按钮打开:`, openErr);
          }
        }
        ...
        oauthActiveRef.current = false;
        await service.completeLogin(resp.loginId);
  ...
  const handleRetryOauth = useCallback(() => {
    ...
    oauthAttemptSeqRef.current += 1;
    if (previousLoginId) {
      oauthServiceRef.current?.cancelLogin(previousLoginId).catch(() => {});
    }
    oauthActiveRef.current = false;
    oauthLoginIdRef.current = null;
    oauthCompletingRef.current = false;
  ...
  const handleOpenOauthUrlWithMode = useCallback(async (incognito: boolean) => {
    ...
    try {
      if (oauthServiceRef.current?.openAuthUrl) {
        await oauthServiceRef.current.openAuthUrl(oauthUrl, incognito);
      } else {
